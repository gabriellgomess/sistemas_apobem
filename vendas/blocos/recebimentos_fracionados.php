<div class="linha">
	<h3 class="mypets2">Recebimentos Fracionados:</h3>
	<div class="thepet2">
		<?php 
		if (($row["vendas_proposta"])&&($row["vendas_proposta"]>1)){
			$result_transacoes = mysql_query("SELECT * FROM sys_vendas_transacoes 
			WHERE transacao_proposta = " . $row['vendas_proposta'] . " OR transacao_proposta = " . $row['vendas_id'] . " 
			ORDER BY transacao_data, transacao_parcela DESC;") 
			or die(mysql_error());
		}
		?>
