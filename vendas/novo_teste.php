...
<script type="text/javascript" src="/sistema/sistema/vendas/js/consulta_apolices_nas_vendas_ajax.js"></script>
...
function meuaccordion(element){

	if($(element).next().hasClass("escondido")){
		$(".visivel").each(function(element){
			$(this).addClass("escondido");
		});
		$(element).next().removeClass("escondido");
	}else{
		$(".visivel").each(function(element){
			$(this).addClass("escondido");
		});
...
function addDivida()
{
	...
	if(contDivida==1){
		document.getElementById('remove_btn_divida').innerHTML = '<input type="button" value="Remove Dívida" onclick="removeDivida();" />';
	}
	...
	if(contDivida<=10)
	{
		...
		elem.innerHTML = "<span id='linha"+contDivida+"'><div style='width:50%; float: left;'><select style='float:none;' name='compra_banco"+contDivida+"'>"+document.getElementById("conteudo_select").innerHTML+"</select></div><div style='width:50%'>Nº do Contrato: <input type='text' id='compra_contrato"+contDivida+"' name='compra_contrato"+contDivida+"' placeholder='contrato a ser comprado' onKeyPress='return SomenteNumero(event)'></div><br /><div style='width:50%; float: left;'>Parcela: <input type='text' id='compra_valor"+contDivida+"' name='compra_valor"+contDivida+"' maxlength='10' size='10' placeholder='valor da parcela' onKeyPress="+"return(MascaraMoeda(this,'.',',',event))"+"></div><div style='width:50%'>Saldo: <input type='text' id='compra_saldo"+contDivida+"' name='compra_saldo"+contDivida+"' maxlength='10' size='10' placeholder='saldo devedor' onKeyPress="+"return(MascaraMoeda(this,'.',',',event))"+"></div><br /><div style='width:50%; float: left;'>Prazo do Contrato: <input type='text' id='compra_prazo"+contDivida+"' name='compra_prazo"+contDivida+"' maxlength='2' size='2' onKeyPress='return SomenteNumero(event)'/></div><div style='width:50%'>Parcelas em aberto: <input type='text' id='compra_parcelas"+contDivida+"' name='compra_parcelas"+contDivida+"' maxlength='2' size='2' onKeyPress='return SomenteNumero(event)'/></div><br /><div style='width:50%; float: left;'>Vencimento: <input type='text' id='compra_venc"+contDivida+"' name='compra_venc"+contDivida+"' maxlength='10' size='10' placeholder='dd/mm/aaaa'/></div><div style='width:50%'>&nbsp;</div><br /><hr><br /></span>";
	...
	if(document.getElementsByClassName("removivel").length>0)
		{
			...
			contDivida--;
			if(contDivida==1){document.getElementById('remove_btn_divida').innerHTML = ''};
		};
	if(document.getElementById("add_btn_divida").innerHTML == "" && cont<11)
	{
		document.getElementById("add_btn_divida").innerHTML = '<input type="button" value="Adicionar Dívida" onclick="addDivida();" />'
	}
}
...
function addCampoSeguro()
{
	...
	if(contSeguro==1){
		document.getElementById('remove_btn_seguro').innerHTML = '<input type="button" value="Remove Apólice" onclick="removeCampoSeguro();" />';
	}
	
	if(contSeguro<=10)
	{
		elem = document.createElement("SPAN");
		elem.className = "removivel_seguro";
		elem.innerHTML = 
'<div id="linha_seguro'+contSeguro+'" style="width: 90%; margin: 10px 0;">'+
'<h3 onclick="meuaccordion(this)" class=" mypets">'+contSeguro+'. Dados da Apólice:</h3>'+
'<div class="visivel '+visibilidade+'">'+
'<table width="99%" class="blocos">'+
'    <tr>'+
'		<td><div align="right"><label for="vendas_banco'+contSeguro+'">Seguradora:</label></div></td>'+
'		<td><div align="left">'+
'			<select id="vendas_banco'+contSeguro+'" name="vendas_banco'+contSeguro+'" onchange="consultaApolicesAjax('+contSeguro+')">'+
...
'		</td>'+
'    </tr>'+
'    <tr id="response-ajax-apolices'+contSeguro+'">'+
'    </tr>'+
'	<tr>'+
'        <td colspan="2">'+
...
'	</tr>'+
'              <tr>'+
'                  <td><div align="right"><label for="vendas_pgto'+contSeguro+'">Forma de Pagamento:</label></div></td>'+
'				  <td><div align="left">'+
'					<select id="vendas_pgto'+contSeguro+'" name="vendas_pgto'+contSeguro+'" onchange="consultaFormaPagamentoAjax('+contSeguro+')">'+
'	<option value="" selected> --- Selecione --- </option>'+
<?php
...
'            </tr>'+
'	    <tr>'+
'	    	<td colspan="2" id="response-ajax-formapagamento'+contSeguro+'">'+
'	    	</td>'+
'   	</tr>'+
'	<tr>'+
'       <td colspan="2">'+
'<h3>Beneficiários:</h3>'+
'	<br />'+
'	<div id="campo_beneficiarios'+contSeguro+'">'+
'	</div>'+	
'	<span id="add_btn_beneficiario'+contSeguro+'"><input type="button" value="Adicionar Beneficiário" onclick="addCampoBeneficiario('+contSeguro+');" /></span>'+
'	<span id="remove_btn_beneficiario'+contSeguro+'"></span>'+
'		</td>'+
'	</tr>'+
'	<tr>'+
'		<td colspan="2">'+
'			<label for="vendas_obs'+contSeguro+'">Observações:</label></br>'+
'			<textarea id="vendas_obs'+contSeguro+'" name="vendas_obs'+contSeguro+'" cols="70" rows="3" id="obs"></textarea>'+
'		</td>'+
'	</tr>'+
'	<tr>'+
'		<td colspan="2" id="botao_salvar'+contSeguro+'">'+
...
'</div>'+
'</div>';
		document.getElementById("campo_seguros").appendChild(elem);

		contSeguro++;
	}
	if(contSeguro==11)
	{
		document.getElementById("add_btn_seguro").innerHTML = "";
	}
}

function removeCampoSeguro()
{
	if(document.getElementsByClassName("removivel_seguro").length>0)
		{
			len = document.getElementsByClassName("removivel_seguro").length;
			document.getElementsByClassName("removivel_seguro")[len-1].remove();
			contSeguro--;
			if(contSeguro==1){document.getElementById('remove_btn_seguro').innerHTML = ''};
		};
	if(document.getElementById("add_btn_seguro").innerHTML == "" && contSeguro<11)
	{
		document.getElementById("add_btn_seguro").innerHTML = '<input type="button" value="Adicionar Apólice" onclick="addCampoSeguro();" />'
	}
}

/* ADICIONAR BENEFICIÁRIO */

var contBenef = new Array();
for (var i = 1; i <11; i++) {
	contBenef[i] = 1;
}

function addCampoBeneficiario(nseg)
{

	if(contBenef[nseg]==1){
		document.getElementById('remove_btn_beneficiario'+nseg).innerHTML = '<input type="button" value="Remover Beneficiário" onclick="removeCampoBeneficiario('+nseg+');" />'
	}
	
	if(contBenef[nseg]<=20)
	{
		elem = document.createElement("div");
		elem.className = "classBeneficiario removivelBeneficiario"+nseg;
		elem.innerHTML = "<span id='linha"+nseg+"-"+contBenef[nseg]+"'><div><div>Nome do Beneficiário: </div></div><div><input type='text' id='ben_nome"+nseg+"-"+contBenef[nseg]+"' name='ben_nome"+nseg+"-"+contBenef[nseg]+"'></div><div><div>Data da Nascimento: </div></div><div><input type='text' id='ben_nasc"+nseg+"-"+contBenef[nseg]+"' name='ben_nasc"+nseg+"-"+contBenef[nseg]+"' placeholder='dd/mm/aaaa' maxlength='10' size='10'></div><div><div>Parentesco: </div></div><div><input type='text' id='ben_parent"+nseg+"-"+contBenef[nseg]+"' name='ben_parent"+nseg+"-"+contBenef[nseg]+"' maxlength='40' size='20'></div><div><div>Percentual: </div></div><div><input type='text' id='ben_perc"+nseg+"-"+contBenef[nseg]+"' name='ben_perc"+nseg+"-"+contBenef[nseg]+"' maxlength='5' size='5' onKeyPress="+"return(MascaraMoeda(this,'.',',',event))"+"></div></span>";
		document.getElementById("campo_beneficiarios"+nseg).appendChild(elem);

		contBenef[nseg]++;
	}
	if(contBenef[nseg]==21)
	{
		document.getElementById("add_btn_beneficiario"+nseg).innerHTML = "";
	}
}

function removeCampoBeneficiario(nseg)
{
	if(document.getElementsByClassName("removivelBeneficiario"+nseg).length>0)
		{
			len = document.getElementsByClassName("removivelBeneficiario"+nseg).length;
			document.getElementsByClassName("removivelBeneficiario"+nseg)[len-1].remove();
			contBenef[nseg]--;
			if(contBenef[nseg]==1){document.getElementById('remove_btn_beneficiario'+nseg).innerHTML = ''};
		};
	if(document.getElementById("add_btn_beneficiario"+nseg).innerHTML == "" && contBenef[nseg]<=20)
	{
		document.getElementById("add_btn_beneficiario"+nseg).innerHTML = '<input type="button" value="Adicionar Beneficiário" onclick="addCampoBeneficiario('+nseg+');" />'
	}
}
/* ADICIONAR BENEFICIÁRIO FIM */
...
function consultaAjax() {
        ...
        xmlhttp.open("GET", "sistema/consultas_ajax/consulta.php?vendas_orgao="+vendas_orgao+"&vendas_produto="+vendas_produto+"&vendas_banco="+vendas_banco+"&vendas_tipo_contrato="+vendas_tipo_contrato+"&vendas_percelas="+vendas_percelas+"&vendas_tabela="+vendas_tabela+"&vendas_valor_parcela="+vendas_valor_parcela+"&vendas_valor="+vendas_valor, true);
        xmlhttp.send();    
}
...
for(i=0; i<document.getElementsByTagName("select").length; i++)
    ...
    if(counter==0)
    {
    	document.getElementById("save_button").innerHTML = '<button name="salvar" type="submit" value="salvar">Salvar Venda</button>';
    }
...
if ($_GET["cpf_geral"]){
	...
	$result_geral = mysql_query("SELECT cliente_nome, cliente_margem, cliente_empregador, cliente_campanha FROM sys_inss_clientes WHERE cliente_cpf = '" . $cpf_geral . "';") 
	or die(mysql_error());
	$row_geral = mysql_fetch_array( $result_geral );
	$cliente_margem = $row_geral["cliente_margem"];
	$vendas_orgao = $row_geral["cliente_empregador"];
...
elseif ($_GET["cpf_mex"]){
	...
	$result_mex = mysql_query("SELECT clients_nm, clients_margem FROM sys_clients WHERE clients_cpf = '" . $cpf_mex . "';") 
	or die(mysql_error());
	$row_mex = mysql_fetch_array( $result_mex );
	$cliente_margem = $row_mex["clients_margem"];
}
		...
		<td><div align="center">
			...
			<h5 class="mypets">SELECIONAR CLIENTE</h5><div class="thepet">
				<table>
					<tr>
<?php if ($cpf_geral): ?>
	<td><div align="center"><a href="index.php?option=com_k2&view=item&layout=item&id=64&Itemid=479&clients_employer=INSS&clients_cpf=<?php echo $cpf_geral;?>&clients_nm=<?php echo $row_geral["cliente_nome"];?>"><?php echo $row_geral["cliente_nome"];?><br />
...
<?php elseif ($cpf_mex): ?>
	<td><div align="center"><a href="index.php?option=com_k2&view=item&layout=item&id=64&Itemid=479&clients_employer=Exercito&clients_cpf=<?php echo $cpf_mex;?>&clients_nm=<?php echo $row_mex["clients_nm"];?>"><?php echo $row_mex["clients_nm"];?><br />
...
<?php if(($_GET["clients_cpf"])&&(($consultores_vendas_internet)||($diretoria))): ?>
	<tr>
...
<?php elseif($row_geral["cliente_campanha"] == "84"): ?>
	<input name="vendas_origem" type="hidden" value="2" />
<?php endif; ?>
	<tr>
		<td><div align="right">			
...
<?php if ($coordenador_plataformas == 1):?>
		<td><div align="left">
...
<?php			
	...
	WHERE sys_equipes.equipe_coordenador = '".$user_id."' 
	ORDER BY name;")
	or die(mysql_error());
...
<table width="99%" class="blocos">	
	...
	<span id="campo_seguros">
	</span>

	<span id="add_btn_seguro"><input type="button" value="Adicionar Apólice" onclick="addCampoSeguro();" /></span>
	<span id="remove_btn_seguro"></span>
	<br /><br />
...
<div class="thepet">
	...
	<span id="add_btn_divida"><input type="button" value="Adicionar Dívida" onclick="addDivida();" /></span>
