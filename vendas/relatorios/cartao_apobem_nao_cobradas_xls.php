...
WHERE `vendas_pgto` = 2 
AND `vendas_banco` = 11 
AND `vendas_status` = ".$_GET['vendas_status'].$filtros_vendas." 
AND NOT EXISTS (SELECT transacao_id FROM sys_vendas_transacoes_seg WHERE transacao_id_venda = sys_vendas_seguros.vendas_id".$filtros_transacoes.")
GROUP BY vendas_id ORDER BY vendas_id DESC;") 
or die(mysql_error());
 ?>
<table border="1" align="center" cellpadding="0" cellspacing="1">
	...
	<?php while($row = mysql_fetch_array( $result )): ?>
		<tr>
			...
			<td><?php echo $row["vendas_dia_ativacao"]; ?></td>
		</tr>
	<?php endwhile; ?>
</table>