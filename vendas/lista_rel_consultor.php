...
while($row = mysql_fetch_array( $result )) {
	...
	AND vendas_mes = '".$row['vendas_mes']."';")
	or die(mysql_error());
	$row_base_consultor = mysql_fetch_array( $result_base_consultor );

	if (($row_user["data_contrato_90"])&&($row_user["data_contrato_90"] != "0000-00-00")&&($row["vendas_dia_pago"])){
		if ($row['vendas_dia_pago'] > $row_user["data_contrato_90"]){$consultor_situacao = 4;}else{$consultor_situacao = 1;}
...
tabela_tipo FROM sys_vendas_tabelas 
...
$vendas_fortcoins = ($cms_total - $row['vendas_taxa']) * $row_tabela["vendas_banco_fortcoins"];
