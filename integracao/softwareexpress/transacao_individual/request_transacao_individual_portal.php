...
$result_transacoes_tef_dia = mysqli_query($con, "SELECT COUNT(transacao_id) AS total FROM sys_vendas_transacoes_tef WHERE transacao_cliente_cpf = '" . $_POST['cpf'] . "' AND transacao_data > '".date('Y-m-d')." 00:00:00';") 
...
function insereRegistroTransacao($con, $dados) {
    ...
    if (!$result_transaction) {
        ...
        // Faça o que for necessário para lidar com o erro
        return $msg;
    }
...
function atualizaTransacao($con, $dados)
{
	...
	SET transacao_nit = '".mysqli_real_escape_string($con,$dados->transacao_nit)."',
	transacao_status = '".mysqli_real_escape_string($con,$dados->transacao_status)."'
	order_id = '".mysqli_real_escape_string($con,$dados->order_id)."'
	WHERE transacao_id = '".mysqli_real_escape_string($con,$dados->transacao_id)."'";
