...
<div class="linha">
	<h3 class="mypets2">Operacional da Venda [Agente]:</h3>
	<div class="thepet2">
		<div class="linha">
			<div class="coluna campo-titulo">Status:</div>
			<div class="coluna campo-valor">
				<?php if ((!$gerente_comercial_agentes)||($row_status_nm["status_liberado"] == 1)):?>	                 
				...
				<?php else:?>
				    <strong><?php echo $row_status_nm["status_nm"];?></strong>
				<?php endif;?>
			</div>

			<div class="coluna campo-titulo">Base Contrato:<br>(somente números)</div>
			<div class="coluna campo-valor">
				R$ <input type="text" name="vendas_base_contrato" value="<?php echo $vendas_base_contrato;?>" size="15" onKeyPress="return(MascaraMoeda(this,'.',',',event))" <?php if (($row['vendas_status'] == "9")&&(!$diretoria)){echo "readonly='true'";} ?>/>				
			</div>
		</div>
		...
		<div class="linha">
			...
			<div class="coluna campo-valor">
                ...
                <p class="lastup"><input type="text" class="w8em format-d-m-y highlight-days-67" id="dp-normal-5" name="dp-normal-5" maxlength="10" size="10" readonly="true" value="<?php echo $vendas_dia_imp;?>" /></p>
			</div>
			...
			<div class="coluna campo-valor">
                ...
                <p class="lastup"><input type="text" class="w8em format-d-m-y highlight-days-67" id="dp-normal-6" name="dp-normal-6" maxlength="10" size="10" readonly="true" value="<?php echo $vendas_dia_pago;?>" /></p>
			</div>
		</div>
		...
		<div class="linha">
			...
			<div class="coluna campo-valor">
				<?php if ($row['vendas_mes'] == ""){echo "Não Informado";}else{echo $row['vendas_mes'];} ?>
			</div>
		</div>

		<?php if ($admin_fisicos == 1 || $diretoria == 1) :?>
			<div class="linha">
	...
	<?php if ($administracao == 1) :?>
        	...
        	<div class="coluna campo-valor">
	    		<select name="vendas_envio">
					<?php
						...
						$result_envio = mysql_query("SELECT * FROM sys_vendas_envio WHERE vendas_envio_tipo=1 ORDER BY envio_id;")
						or die(mysql_error());
	    ...
	    <div class="linha">
			<?php if ($row['vendas_envio'] == 1): ?>
							...
							<div class="coluna campo-valor">
								<input value="<?php echo $row['vendas_envio_objeto']; ?>" name="vendas_envio_objeto" type="text" size="20" maxlength="30"/>
							</div>
							<div class="coluna campo-titulo">Nome da empresa:</div>
							<div class="coluna campo-valor">
								<input value="<?php echo $row['vendas_envio_empresa']; ?>" name="vendas_envio_empresa" type="text" size="20" maxlength="30"/>
							</div>
			<?php else:?>
							...
							<div class="coluna campo-valor">
								<input type="text" class="w8em format-d-m-y highlight-days-67" id="dp-normal-7" name="dp-normal-7" maxlength="10" size="10" readonly="true" value="<?php echo $vendas_envio_data;?>" />
							</div>
			<?php endif;?>
		</div>
		...
		<?php if ($row['vendas_envio_objeto']): ?>
			<div class="linha">
				<a href="http://www.linkcorreios.com.br/?id=<?php echo $row['vendas_envio_objeto']; ?>" rel="lyteframe" rev="width: 600px; height: 400px; scroll:no;" title="Rastreio de Objeto"><img src="sistema/imagens/correios.png"><br />
				Rastrear Objeto</a>
			</div>
		<?php endif;?>
		...
		<div class="linha">
			<?php if ($row['vendas_retorno'] == 1): ?>		
					<div class="coluna campo-titulo">Contato:</div>
					<div class="coluna campo-valor">
						<input value="<?php echo $row['vendas_retorno_objeto']; ?>" name="vendas_retorno_objeto" type="text" size="20" maxlength="30"/><br />
					</div>
					<div class="coluna campo-titulo">Nome da empresa:</div>
					<div class="coluna campo-valor">
						<input value="<?php echo $row['vendas_retorno_empresa']; ?>" name="vendas_retorno_empresa" type="text" size="20" maxlength="30"/>
					</div>
			<?php else:?>
					<div class="coluna campo-titulo">Nº do objeto:</div>
					<div class="coluna campo-valor">
						<input value="<?php echo $row['vendas_retorno_objeto']; ?>" name="vendas_retorno_objeto" type="text" size="20" maxlength="30"/>
					</div>
					...
					<div class="coluna campo-titulo">Data de envio:</div>
					<div class="coluna campo-valor">
						<input type="text" class="w8em format-d-m-y highlight-days-67" id="dp-normal-8" name="dp-normal-8" maxlength="10" size="10" readonly="true" value="<?php echo $vendas_retorno_data;?>" />
					</div>
			<?php endif;?>
		</div>
		...
		<?php if ($row['vendas_retorno_objeto']): ?>
			<div class="linha">
					<a href="http://websro.correios.com.br/sro_bin/txect01$.QueryList?P_LINGUA=001&P_TIPO=001&P_COD_UNI=<?php echo $row['vendas_retorno_objeto']; ?>" rel="lyteframe" rev="width: 600px; height: 400px; scroll:no;" title="Rastreio de Objeto"><img src="sistema/imagens/correios.png"><br />
					Rastrear Objeto</a>
			</div>							
		<?php endif;?>
