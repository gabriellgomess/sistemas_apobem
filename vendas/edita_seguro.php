		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				...
				jQuery(".idLink").click(function() {
					...
					if (jQuery("#includes_user_id").val() == 1004 || jQuery("#includes_user_id").val() == 129 || jQuery("#includes_user_id").val() == 4177 || jQuery("#includes_user_id").val() == 42 || jQuery("#includes_user_id").val() == 3510) {
...
*/
			...
			<div
				...
				"
				id="modalLoading">
				<div style="width: 350px; height: 200px; background-color: #fff; border-radius: 10px; display: flex; justify-content: center; align-items: center;">
...
include("sistema/utf8.php");
	...
	<script type="text/javascript">
		...
		jQuery.ajax({
			...
			success: function(response) {
				// Aqui, o response já é um objeto, então não precisa de JSON.parse
				console.log(response);

				// Verifica se existem registros
				if (response.totalCount > 0 && response.data.length > 0) {
					// Pega o primeiro registro da lista
					var firstRecord = response.data[0];
					console.log(firstRecord);

					// Oculta elementos se existir pelo menos um registro
					jQuery("#emailTitulo").hide();
					jQuery("#cliente_email_editar_div").hide();
					jQuery("#tituloFone").hide();
					jQuery("#cliente_fone_editar").hide();
				} else {
					// Exibe os elementos se não houver registros
			...
			if (dueDateInput > d) {
				jQuery.ajax({
					...
					success: function(response) {
						...
						// Verifica se `response.totalCount` existe e é maior que 0
						if (response.totalCount > 0) {
							var lista = response.data;
							var id_custumores_asaaz = null;
							...
							jQuery.ajax({
								...
								data: {
									...
									jsonReponse: response // Usa `response` diretamente
								},
		...
		function CriaBoleto(idAsaas, idTransacao) {
			...
			jQuery.ajax({
				...
				success: function(resposta) {
					...
					var jsonResponse = JSON.parse(resposta);
					...
					var id_boleto = jsonResponse.id;
					var invoiceUrl = jsonResponse.invoiceUrl;
					var bankSlipUrl = jsonResponse.bankSlipUrl;
					var invoiceNumber = jsonResponse.invoiceNumber;
					...
					jQuery.ajax({
						...
						data: {
							...
							idAsaas: idAsaas,
							value: value,
							jsonReponse: resposta, // Envia a resposta original como string JSON
							vendas_id: vendas_id,
							userid: userid,
							cpfCnpj: cpfCnpj,
						...
						success: function(resposta) {
							...
							// Atualiza o link do boleto na interface
