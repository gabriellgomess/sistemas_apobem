...
// consulta se o mês da venda já existe e, caso não exista, cadastra o mês da venda.
	cadastraNovoMes($vendas_mes);


// consulta os dados da venda antes de atualizá-la
$result_old = mysql_query("SELECT clients_cpf, vendas_pos_venda, vendas_proposta, vendas_status, vendas_tipo_contrato, vendas_tabela, vendas_banco, vendas_envio_objeto, vendas_contrato_fisico, vendas_base_prod FROM sys_vendas WHERE vendas_id = '" . $vendas_id . "';") 
or die(mysql_error());  
...
<?php else: ?>
	...
	if($vendas_contrato_fisico)
	{
		if($vendas_contrato_fisico != $row_old["vendas_contrato_fisico"])
		{
			$vendas_contrato_fisico=$vendas_contrato_fisico;
	...
	if($sup_operacional == 1)
	{
		...
		vendas_alteracao='$vendas_alteracao', 
		vendas_user='$vendas_user'".$update_status.$update_promotora.$update_fisico.$update_fisico2.$update_pos_venda.$update_tabela.$update_proposta.$update_coeficiente.$update_banco.$update_tipo_contrato.$update_percelas.$update_envio_valor.$update_vendas_unidade.$vendas_produto_update."
		WHERE vendas_id='$vendas_id' ") or die(mysql_error());
		echo "Venda Atualizada com Sucesso";
		...
		vendas_intencionada='$vendas_intencionada', 
		vendas_alteracao='$vendas_alteracao', 
		vendas_user='$vendas_user'".$update_status.$update_promotora.$update_pos_venda.$vendas_produto_update."
		WHERE vendas_id='$vendas_id' ") or die(mysql_error());
		echo "Venda Atualizada com Sucesso";
	}
...
// FUNÇÕES
// consulta se o mês da venda já existe e, caso não exista, cadastra o mês da venda.
function cadastraNovoMes($vendas_mes)
{
