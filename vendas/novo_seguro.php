...
<script type="text/javascript" src="/sistema/sistema/js/jquery-2.1.4.min.js"></script>

<input type="hidden" id='user_id' value="<?php echo $user_id;?>"/>
<input type="hidden" id='username' value="<?php echo $_GET['username'];?>"/>

<script type="text/javascript" src="/sistema/sistema/vendas/js/softwareexpress/verifica_cartao.js?<?php echo filemtime("/sistema/sistema/vendas/js/softwareexpress/verifica_cartao.js"); ?>"></script>
...
//-----------------------------------------------------
function calculoValor(element){

	valor = element.value.replace(/\./g,"");
	valor = valor.replace(/\,/g,".");
	valor = parseFloat(valor) * 0.02;	
	if(valor > 30) valor = 30;
	valor = valor.toFixed(2);	
	document.getElementById("apolice_valor").value = valor;
	document.getElementById("valorC").innerHTML = valor.replace(/\./g,",");
}

function campoValor(valor){
	if(parseFloat(valor) > 0)
	{
		document.getElementById("campo_salario").innerHTML = '';
		document.getElementById("campo_valor").innerHTML = '';
	}else{
		//document.getElementById("campo_salario").innerHTML = 'Salário: R$ <input type="text" id="salario_bruto" name="salario_bruto" value="" onkeyup="calculoValor(this);" onkeypress="return(MascaraMoeda(this,\'.\',\',\',event));">';
		//document.getElementById("campo_valor").innerHTML = 'Valor: R$ <span id="valorC"></span><input type="hidden" id="apolice_valor" name="apolice_valor" value="">';
		document.getElementById("campo_salario").innerHTML = 'Valor: R$ <input type="text" id="apolice_valor" name="apolice_valor" value="" onkeypress="return(MascaraMoeda(this,\'.\',\',\',event));">';
	}
}
...
function formaPgto(fp){
	...
	//document.getElementById('vendas_apolice').innerHTML+="<pre class='nana' style='display:none'>"+vendas_apolice+"</pre>"
/*
		...
		var val = fp;
		  for (var opt, j = 0; opt = opts[j]; j++) {
		    if (opt.value == val) {
		      sel.selectedIndex = j;		      
		      sel.disabled = true;
		      document.getElementById('vendas_pgto').onchange();
		      break;
		    }
		  }
...
while($row_grupo_user = mysql_fetch_array( $result_grupo_user )){
	...
	if ($row_grupo_user['id'] == '103'){$cobranca_seguros = 1;}
}

if ($empresa_fortune == 1){$apolice_tipo = 2;}else{$apolice_tipo = 1;}
if ($cobranca_seguros == 1 || $administracao == 1){$apolice_tipo = 0;}
...
<form id="testform" action="index.php" method="GET">
					...
					<input name="token_transactions" type="hidden" id='token_transactions' value="<?php echo $_GET['token'];?>"/>
...
<?php if ( ( $administracao == 1 || $coordenador_plataformas == 1) ):?>
		<td><div align="left">
...
<?php endif;?>
	<tr>
		<td><div align="right"><label for="forma_envio_kitcert">Forma Envio Kit-Certificado:</label></div></td>
		<td><div align="left">
    ...
    <tr>
		...
		<td><div align="left">
				...
				<?php
					...
					$result_banco = mysql_query("SELECT * FROM sys_vendas_banco_seg WHERE banco_ativo = 1 ORDER BY banco_nm;") or die(mysql_error());
					
					while($row_banco = mysql_fetch_array( $result_banco ))
					{
						$result_apolice = mysql_query("SELECT * 
														FROM sys_vendas_apolices 
														WHERE apolice_banco = '".$row_banco["banco_id"]."' 
															AND apolice_ativa=1 
															AND apolice_tipo = '".$apolice_tipo."' 
															ORDER BY apolice_nome;") or die(mysql_error());
						$apolice_num_rows = mysql_num_rows($result_apolice);
						if($apolice_num_rows > 0 ) // Caso o banco possua alguma apólice.
						{
...
<h3 class="mypets">Dados Financeiros:</h3>
               ...
               <tr id="response-ajax-apolice-pgto">
...
<h3 class="mypets">Beneficiários:</h3>
	...
	<tr>
<?php if (false/*$apolice_tipo == 1*/):?>
		<td colspan="2" id="botao_salvar">
			<div style='color: red'>Preencha os campos corretamente para salvar.</div>
		</td>
<?php else:?>
		<td colspan="2">
			<div style='color: red'><button name='salvar' type='submit' value='salvar' onclick='salvando_modal()'>Salvar Venda</button></div>
		</td>
<?php endif;?>
	</tr>
</table>
