...
if ($_GET["clients_cpf"]){
	...
	if (!$row_geral["cliente_cpf"]){
		$result_mex = mysql_query("SELECT clients_nm, clients_margin FROM sys_clients WHERE clients_cpf = '" . $clients_cpf . "';") 
		or die(mysql_error());
		$row_mex = mysql_fetch_array( $result_mex );
		$cliente_margem = $row_mex["clients_margin"];
	}
}

if ($_GET["cpf_geral"]){
	$cpf_geral = $_GET["cpf_geral"];
	$result_geral = mysql_query("SELECT cliente_nome, cliente_margem FROM sys_inss_clientes WHERE cliente_cpf = '" . $cpf_geral . "';") 
	or die(mysql_error());
	$row_geral = mysql_fetch_array( $result_geral );
	$cliente_margem = $row_geral["cliente_margem"];
}
elseif ($_GET["cpf_mex"]){
	...
	$result_mex = mysql_query("SELECT clients_nm, clients_margin FROM sys_clients WHERE clients_cpf = '" . $cpf_mex . "';") 
	or die(mysql_error());
	$row_mex = mysql_fetch_array( $result_mex );
	$cliente_margem = $row_mex["clients_margin"];
}
...
<?php if ($user_id == 42):?>
	...
	for (iLoop = 1; iLoop <= totalCampos; iLoop++) {
		document.write("<span id='linha"+iLoop+"' style='display:none'><div style='width:50%; float: left;'><select style='float:none;' name='compra_banco_"+iLoop+"'><?php echo $opcoes_select_banco; ?></select></div><div style='width:50%'>Nº do Contrato: <input type='text' id='compra_contrato"+iLoop+"' name='compra_contrato"+iLoop+"' placeholder='contrato a ser comprado' onKeyPress='return SomenteNumero(event)'></div><br />");
		document.write("<div style='width:50%; float: left;'>Parcela: <input type='text' id='compra_valor"+iLoop+"' name='compra_valor"+iLoop+"' maxlength='10' size='10' placeholder='valor da parcela' onKeyPress='return(MascaraMoeda(this,'.',',',event))'></div><div style='width:50%'>Saldo: <input type='text' id='compra_saldo"+iLoop+"' name='compra_saldo"+iLoop+"' maxlength='10' size='10' placeholder='saldo devedor' onKeyPress='return(MascaraMoeda(this,'.',',',event))'></div><br />");
		document.write("<div style='width:50%; float: left;'>Prazo do Contrato: <input type='text' id='compra_prazo"+iLoop+"' name='compra_prazo"+iLoop+"' maxlength='2' size='2' onKeyPress='return SomenteNumero(event)'></div><div style='width:50%'>Parcelas em aberto: <input type='text' id='compra_parcelas"+iLoop+"' name='compra_parcelas"+iLoop+"' maxlength='2' size='2' onKeyPress='return(MascaraMoeda(this,'.',',',event))'></div><br />");
		document.write("<div style='width:50%; float: left;'>Vencimento: <input type='text' id='compra_venc"+iLoop+"' name='compra_venc"+iLoop+"' maxlength='10' size='10' placeholder='dd/mm/aaaa'></div><div style='width:50%'>&nbsp;</div><br /><hr><br /></span>");
	}
	</script>
