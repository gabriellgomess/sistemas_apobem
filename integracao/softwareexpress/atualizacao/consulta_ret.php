...
$result_transacoes = mysqli_query($con, "SELECT sys_vendas_transacoes_tef.transacao_id, transacao_nit, transacao_venda_id, transacao_cliente_cpf, transacao_data FROM sys_vendas_transacoes_tef 
											INNER JOIN sys_vendas_transacoes_tef_log ON sys_vendas_transacoes_tef.transacao_id = sys_vendas_transacoes_tef_log.transacao_id 
											WHERE status = 'RET' AND (transacao_status = 'RET' OR transacao_status = 'INV') LIMIT 0, 10;")
											or die(mysqli_error($con));

if(mysqli_num_rows($result_transacoes)){
	while($row_transacoes = mysqli_fetch_array($result_transacoes)){
		$nit = $row_transacoes["transacao_nit"];
	...
	echo "<pre>Nenhuma transação em retentativa.</pre>";
}
?>