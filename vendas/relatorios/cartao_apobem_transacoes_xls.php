...
if($_GET['transacao_recebido']){$filtro_recebido = " AND transacao_recebido = ".$_GET['transacao_recebido'];}
...
WHERE `vendas_pgto` = 2 AND `vendas_banco` = 11 
AND `vendas_status` = ".$_GET['vendas_status'].$filtros_vendas.$filtros_transacoes.$filtro_recebido." GROUP BY vendas_id ORDER BY vendas_id DESC;") 
or die(mysql_error());
...
<table border="1" align="center" cellpadding="0" cellspacing="1">
	<tr>
		...
		<td>VALOR</td>
		<td>DATA VENDA</td>
		<td>DATA TRANSACAO</td>
		<td>DATA IMPORTACAO</td>
		<td>VALOR TRANSACAO</td>
		<td>PARCELA</td>
		<td>ID TRANSACAO</td>
	</tr>
	<?php while($row = mysql_fetch_array( $result )): ?>
		<tr>
			...
			<td><?php echo $row["transacao_data"]; ?></td>
			<td><?php echo $row["transacao_data_importacao"]; ?></td>
			<td><?php echo $row["transacao_valor"]; ?></td>
			<td><?php echo $row["transacao_parcela"]; ?></td>
