			...
			<div align="center" style="text-align: center; font-weight: bold; width: 100%; margin: auto;">
				<table id="comissoes_tbl" width="100%">
					<tr>
						...
						<th>Impostos:</th>
						<th>Sub-total R$:</th>
						<?php if(!$consultor_mei): ?><th>Sub-total BS¢:</th><?php endif; ?>
					</tr>
					<?php 
					if ($consultor_mei){$filtro_sql_receitas = " AND cms_tipo_recebimento=1";}
					$result_cms_venda = mysql_query("SELECT cms_id, tipo_id, cms_tipo_nome, cms_tipo_recebimento, cms_perc, cms_valor, cms_subtotal, cms_fortcoins, cms_obs, cms_imposto FROM sys_vendas_cms 
					INNER JOIN sys_vendas_cms_tipos ON sys_vendas_cms.tipo_id = sys_vendas_cms_tipos.cms_tipo_id 
					WHERE vendas_id=".$vendas_id.$filtro_sql_receitas.";")
						or die(mysql_error()); 
						$cont=1;
					?>
					...
					<tr class="removivel">
						<td>
							<?php 
							echo $cont; $cont++; 
							$cms_tipo_recebimento = $row_cms_venda['cms_tipo_recebimento'];
							?>
						</td>
						<td>
							<?php echo $row_cms_venda['cms_tipo_nome']; ?>
						</td>
						<td>
							R$ 
							<?php 
							...
							if(($row_cms_venda["tipo_id"] == 13)||($row_cms_venda["tipo_id"] == 14)||($row_cms_venda["tipo_id"] == 15)||($row_cms_venda["tipo_id"] == 16)){
						...
						<td id="antecipacao">
                            <?php if($cms_tipo_recebimento==2): ?>
                                <?php 
                                    $result_cms_antecipacao = mysql_query("SELECT cms_perc, cms_subtotal, cms_fortcoins FROM sys_vendas_cms WHERE vendas_id=".$vendas_id." AND tipo_id=".($row_cms_venda['tipo_id']+1).";")
                                        or die(mysql_error());
						...
						<td>
							<?php if($row_cms_venda['cms_imposto']==1){echo "SIM";}else{echo "NÃO";} ?>
						</td>
						<td>R$ 
							<?php 
						...
						<?php if(!$consultor_mei): ?>
							<td>BS¢ 
								<?php 
									if($cms_tipo_recebimento==2){
										echo number_format($row_cms_antecipacao['cms_fortcoins'], 2, ',', '.');
										$totais_cms_fortcoins = $totais_cms_fortcoins + $row_cms_antecipacao['cms_fortcoins'];
									}else{
										echo number_format($row_cms_venda['cms_fortcoins'], 2, ',', '.');
										if($cms_tipo_recebimento==1){$totais_cms_fortcoins = $totais_cms_fortcoins + $row_cms_venda['cms_fortcoins'];}
									}
								?>
							</td>
						<?php endif; ?>
					...
					<tr>
						...
						<?php if(!$consultor_mei): ?><td>BS¢ <?php echo number_format($totais_cms_fortcoins, 2, ',', '.'); ?></td><?php endif; ?>
					</tr>
				</table>
			</div>
		</div>
...
<?php if(!$consultor_mei): ?>
	...
	<?php endif; ?>
		...
		<?php endif;?>
		<div class="linha">
			<div class="coluna campo-titulo">Definir Base:</div>
			<div class="coluna campo-valor">
				<select name="vendas_base">
				  <option value="0" <?php if ($row['vendas_base'] == 0){echo "selected";}?>>Selecione</option>
				  <option value="1" <?php if ($row['vendas_base'] == 1){echo "selected";}?>>1</option>
				  <option value="2" <?php if ($row['vendas_base'] == 2){echo "selected";}?>>2</option>
				</select>
			</div>
			<div class="coluna campo-titulo">Coeficiente Base 2:</div>
			<div class="coluna campo-valor"><input value="<?php echo $coeficiente_base; ?>" name="coeficiente_base" type="text" size="3" maxlength="3"/> (Ex.: 6,0)</div>
		</div>
		<div class="linha">
		...
		<div class="linha">
			...
			<div class="coluna campo-titulo">Calculos:</div>
			<div class="coluna campo-valor">
				<input type="checkbox" name="calc" value="1"><strong>Calcular Base</strong></br>
                <input type="checkbox" name="calc_rec" value="1"><strong>Calcular Receita</strong>
			</div>
		</div>
		<div class="linha">
		...
		<div class="linha">
			<div class="coluna">
				<?php if (($diretoria == 1)||($userid == 165)) :?>
					<?php $vendas_cms_perc_af = ($row['vendas_cms_perc_af']>0) ? number_format($row['vendas_cms_perc_af'], 2, ',', '.') : '0' ;?>
					</br>Percentual de Receita referente a Base do Contrato: <?php echo $vendas_cms_perc_af;?>%
				<?php endif;?>
			</div>
		</div>
	<?php endif;?>
